  ...
  return (
    <main className="min-h-screen bg-light dark:bg-dark text-dark dark:text-white">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pb-24">
        ...
        {/* Title */}
        <h1 className="font-space text-2xl sm:text-4xl font-bold mb-3">{project.title}</h1>
        <p className="font-inter text-gray-500 dark:text-white/60 text-base mb-6">{project.desc}</p>
        ...
        <div className="bg-white dark:bg-dark-card border border-light-border dark:border-dark-border rounded-2xl p-5 sm:p-6 mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {project.tags.map((tag, i) => (
              <span key={i} className="font-inter text-sm text-cyan bg-cyan/10 border border-cyan/20 px-3 py-1.5 rounded-full">
                {tag}
              </span>
            ))}
          </div>
        ...
        <div className="bg-white dark:bg-dark-card border border-light-border dark:border-dark-border rounded-2xl p-5 sm:p-6 mb-6">
          ...
          <p className="font-inter text-gray-600 dark:text-white/70 leading-relaxed">{project.details.description}</p>
        </div>
        ...
        <div className="bg-white dark:bg-dark-card border border-light-border dark:border-dark-border rounded-2xl p-5 sm:p-6 mb-6">
          ...
          <ul className="space-y-3">
            {project.details.challenges.map((c, i) => (
              <li key={i} className="flex gap-3 font-inter text-gray-600 dark:text-white/70 text-sm">
        ...
        {/* Other Projects */}
        <div>
          <h2 className="font-space text-xl font-semibold mb-4">Other Projects</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            {projects.filter((p) => p.slug !== project.slug).slice(0, 2).map((p, i) => (
              <Link key={i} href={`/projects/${p.slug}`}
                ...
                <div>
                  <h3 className="font-space text-sm font-semibold text-dark dark:text-white group-hover:text-cyan transition-colors">{p.title}</h3>
                  <p className="font-inter text-xs text-gray-500 dark:text-white/40 mt-1">{p.type}</p>
                </div>
              </Link>
            ))}
          </div>
